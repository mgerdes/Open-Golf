...
void main()
{
    vec4 _53 = texture(ce_water_noise_tex1, vec2(frag_texture_coord.x, 0.100000001490116119384765625 * (frag_texture_coord.y + (16.0 * hole_editor_water_fs_params[0].x))));
    vec3 color = vec3(0.0039215688593685626983642578125, 0.4862745106220245361328125, 0.56078433990478515625);
    if (texture(ce_water_noise_tex0, vec2(frag_texture_coord.x, frag_texture_coord.y + hole_editor_water_fs_params[0].x)).w > 0.100000001490116119384765625)
    {
