...
static void entity_array_sort_idxs(vec_editor_entity_t *entity_array) {
    for (int i = 0; i < entity_array->length; i++) {
        ...
        vec_swap(entity_array, smallest_idx, i);
    }
}
...
bool editor_entity_array_contains_entity(vec_editor_entity_t *entity_array,
        struct editor_entity editor_entity) {
    struct editor_entity entity;
    int i;
    vec_foreach(entity_array, entity, i) {
...
static bool is_selectable_array_equal(vec_editor_entity_t *selected_array,
        vec_editor_entity_t *hovered_array) {
...
static void hole_editor_push_state(struct hole_editor *ce) {
    ...
    if (ce->history.idx < ce->history.state_array.length) {
        ...
        for (int i = start; i < start + count; i++) {
            ...
            vec_deinit(&ce->history.state_array.data[i].selected_array);
        }
        vec_splice(&ce->history.state_array, start, count);
    }

    if (ce->history.idx >= 100) {
        data_stream_deinit(&ce->history.state_array.data[0].serialization);
        vec_deinit(&ce->history.state_array.data[0].selected_array);
        vec_splice(&ce->history.state_array, 0, 1);
        vec_push(&ce->history.state_array, state);
    }
    else {
        vec_push(&ce->history.state_array, state);
        ce->history.idx++;
...
static void hole_editor_update_model_generator_scripts(struct hole_editor *ce, bool first_time) {
    vec_model_generator_script_t *scripts = &ce->scripts;
...
static void hole_editor_init(struct hole_editor *ce, struct renderer *renderer) {
    ...
    {
        ce->history.idx = 0;
        vec_init(&ce->history.state_array);
    }
    {
    ...
    }
    {
        ...
        vec_init(&ce->object_modifier_entities);
        vec_init(&ce->object_modifier_start_positions);
        vec_init(&ce->object_modifier_start_orientations);
        ce->select_box.is_open = false;
        ce->select_box.p0 = V2(0.0f, 0.0f);
        ce->select_box.p1 = V2(0.0f, 0.0f);
        vec_init(&ce->selected_array);
        vec_init(&ce->hovered_array);
    }
    {
        ce->file.path[0] = 0;
    ...
    }
    {
        ...
        vec_init(&ce->edit_terrain_model.selected_array);
        vec_init(&ce->edit_terrain_model.hovered_array);
        ...
        vec_init(&ce->edit_terrain_model.copied_points);
        vec_init(&ce->edit_terrain_model.copied_faces);
    }
    {
...

    {
        ...
        for (int i = 0; i < dir.num_files; i++) {
            ...
            vec_init(&script.params);
            vec_init(&script.values);
            script.file = file;
            vec_push(scripts, script);
        }
        mdir_deinit(&dir);
...
static void add_unique_point(vec_int_t *added_points_idxs, vec_int_t *new_points_idxs,
        vec_vec3_t *points, int point_idx, vec3 point, int *new_point_idx) {
    ...
    *new_point_idx = points->length;
    vec_push(added_points_idxs, point_idx);
    vec_push(new_points_idxs, points->length);
...
static void hole_editor_update(struct game_editor *ed, float dt,
    ...
    if (ce->global_illumination.do_it) {
        if (!lightmap_generator_is_running()) {
            ...
            for (int i = 0; i < data->entities.length; i++) {
                ...
                vec_vec2_t lightmap_uvs = lm_entity->lightmap_uvs;

                lightmap->uvs.length = 0;
                for (int i = 0; i < lightmap_uvs.length; i++) {
                    vec_push(&lightmap->uvs, lightmap_uvs.data[i]);
                }
            ...
            for (int i = 0; i < data->multi_entities.length; i++) {
...

                {
                    ...
                    vec_vec2_t lightmap_uvs = lm_entity->static_lightmap_uvs;
                    assert((lightmap_width == lightmap->width) && (lightmap_height == lightmap->height));
...

                {
                    ...
                    vec_vec2_t lightmap_uvs = lm_entity->moving_lightmap_uvs;
                    assert((lightmap_width == lightmap->width) && (lightmap_height == lightmap->height));
    ...
    if (ce->edit_terrain_model.active) {
        vec_editor_entity_t *selected_array = &ce->edit_terrain_model.selected_array;
        vec_editor_entity_t *hovered_array = &ce->edit_terrain_model.hovered_array;
        ...
        if (ce->select_box.is_open) {
            ...
            if (ce->edit_terrain_model.points_selectable) {
                for (int i = 0; i < model->points.length; i++) {
                    ...
                    if (point_inside_box(point, box_center, box_half_lengths)) {
                        ...
                        vec_push(hovered_array, hovered);
                    }
                }
            }
            ...
            if (ce->edit_terrain_model.faces_selectable) {
                ...
                vec_init(&triangle_points);
                vec_init(&triangle_idxs);
                vec_init(&is_inside);
            ...
            if (ce->edit_terrain_model.points_selectable) {
                vec_vec3_t sphere_center;
                vec_float_t sphere_radius;
                vec_init(&sphere_center);
                vec_init(&sphere_radius);

                for (int i = 0; i < model->points.length; i++) {
                    vec3 point = model->points.data[i];
                    point = vec3_apply_mat4(point, 1.0f, model_mat);
                    vec_push(&sphere_center, point);
                    vec_push(&sphere_radius, 0.04f);
                }
                ...
                vec_deinit(&sphere_center);
                vec_deinit(&sphere_radius);
            }
        ...
        if (!om->is_using) {
            if (button_inputs.mouse_clicked[SAPP_MOUSEBUTTON_LEFT]) {
                ...
                for (int i = 0; i < hovered_array->length; i++) {
                    if (!editor_entity_array_contains_entity(selected_array, hovered_array->data[i])) {
                        vec_push(selected_array, hovered_array->data[i]);
                    }
                    else if (shift_down) {
                ...
                for (int i = 0; i < selected_array->length; i++) {
                    ...
                    if (selected.type == EDITOR_ENTITY_POINT) {
                        if (!editor_entity_array_contains_entity(&ce->object_modifier_entities, selected)) {
                            vec_push(&ce->object_modifier_entities, selected);
                            vec_push(&ce->object_modifier_start_positions, model->points.data[selected.idx]);
                            vec_push(&ce->object_modifier_start_orientations, QUAT(1.0f, 0.0f, 0.0f, 0.0f));
                        }
                    }
                        ...
                        if (face.num_points == 3) {
                            ...
                            if (!editor_entity_array_contains_entity(&ce->object_modifier_entities, point0)) {
                                vec_push(&ce->object_modifier_entities, point0);
                                vec_push(&ce->object_modifier_start_positions, model->points.data[point0.idx]);
                                vec_push(&ce->object_modifier_start_orientations, QUAT(1.0f, 0.0f, 0.0f, 0.0f));
                            }
                            if (!editor_entity_array_contains_entity(&ce->object_modifier_entities, point1)) {
                                vec_push(&ce->object_modifier_entities, point1);
                                vec_push(&ce->object_modifier_start_positions, model->points.data[point1.idx]);
                                vec_push(&ce->object_modifier_start_orientations, QUAT(1.0f, 0.0f, 0.0f, 0.0f));
                            }
                            if (!editor_entity_array_contains_entity(&ce->object_modifier_entities, point2)) {
                                vec_push(&ce->object_modifier_entities, point2);
                                vec_push(&ce->object_modifier_start_positions, model->points.data[point2.idx]);
                                vec_push(&ce->object_modifier_start_orientations, QUAT(1.0f, 0.0f, 0.0f, 0.0f));
                            }
                        }
                        else if (face.num_points == 4) {
                            ...
                            if (!editor_entity_array_contains_entity(&ce->object_modifier_entities, point3)) {
                                vec_push(&ce->object_modifier_entities, point3);
                                vec_push(&ce->object_modifier_start_positions, model->points.data[point3.idx]);
                                vec_push(&ce->object_modifier_start_orientations, QUAT(1.0f, 0.0f, 0.0f, 0.0f));
                            }
                        }
                        else {
        ...
        vec_editor_entity_t *hovered_array = &ce->hovered_array;
        vec_editor_entity_t *selected_array = &ce->selected_array;

        profiler_push_section("get_hovered_entity");
        {
            vec_vec3_t triangle_points;
            vec_editor_entity_t entities;
            vec_init(&triangle_points);
            vec_init(&entities);
            ...
            if (min_t < FLT_MAX && !om->is_hovered && !om->is_using) {
                vec_push(hovered_array, entities.data[min_idx]);
            }
            ...
            if (om->set_start_positions) {
                ...
                for (int i = 0; i < selected_array->length; i++) {
                    ...
                    if (selected.type == EDITOR_ENTITY_TERRAIN) {
                        ...
                        vec_push(&ce->object_modifier_entities, selected);
                        vec_push(&ce->object_modifier_start_positions, terrain->position);
                        vec_push(&ce->object_modifier_start_orientations, terrain->orientation);
                    }
                    else if (selected.type == EDITOR_ENTITY_BALL_START) {
                        struct ball_start_entity *ball_start = &hole->ball_start_entity;
                        vec_push(&ce->object_modifier_entities, selected);
                        vec_push(&ce->object_modifier_start_positions, ball_start->position);
                        vec_push(&ce->object_modifier_start_orientations, QUAT(1.0f, 0.0f, 0.0f, 0.0f));
                    }
                    else if (selected.type == EDITOR_ENTITY_CUP) {
                        struct cup_entity *cup = &hole->cup_entity;
                        vec_push(&ce->object_modifier_entities, selected);
                        vec_push(&ce->object_modifier_start_positions, cup->position);
                        vec_push(&ce->object_modifier_start_orientations, QUAT(1.0f, 0.0f, 0.0f, 0.0f));
                    }
                        ...
                        struct beginning_camera_animation_entity *cam_anim = 
                            &hole->beginning_camera_animation_entity;
                        vec_push(&ce->object_modifier_entities, selected);
                        vec_push(&ce->object_modifier_start_positions, cam_anim->start_position);
                        vec_push(&ce->object_modifier_start_orientations, QUAT(1.0f, 0.0f, 0.0f, 0.0f));
                    }
                        ...
                        vec_push(&ce->object_modifier_entities, selected);
                        vec_push(&ce->object_modifier_start_positions, camera_zone->position);
                        vec_push(&ce->object_modifier_start_orientations, camera_zone->orientation);
                    }
                        ...
                        vec_push(&ce->object_modifier_entities, selected);
                        vec_push(&ce->object_modifier_start_positions, entity->moving_position);
                        vec_push(&ce->object_modifier_start_orientations, entity->moving_orientation);
                    }
                    else if (selected.type == EDITOR_ENTITY_MULTI_TERRAIN_STATIC) {
                        struct multi_terrain_entity *entity = 
                            &hole->multi_terrain_entities.data[selected.idx];
                        vec_push(&ce->object_modifier_entities, selected);
                        vec_push(&ce->object_modifier_start_positions, entity->static_position);
                        vec_push(&ce->object_modifier_start_orientations, entity->static_orientation);
                    }
                        ...
                        vec_push(&ce->object_modifier_entities, selected);
                        vec_push(&ce->object_modifier_start_positions, entity->position);
                        vec_push(&ce->object_modifier_start_orientations, entity->orientation);
                    }
...

    {
...

        {
                ...
                if (igBeginMenu("Edit", true)) {
                    ...
                    if (igMenuItemBool("Copy", "Ctrl+C", false, true)) {
                        if (ce->edit_terrain_model.active) {
                            struct terrain_model *model = ce->edit_terrain_model.model;
                            vec_editor_entity_t *selected_array = &ce->edit_terrain_model.selected_array;
                            vec_vec3_t *copied_points = &ce->edit_terrain_model.copied_points;
                            vec_terrain_model_face_t *copied_faces = &ce->edit_terrain_model.copied_faces;
                            vec_int_t added_points_idxs, new_points_idxs;
                            vec_init(&added_points_idxs);
                            vec_init(&new_points_idxs);
                            ...
                            for (int i = 0; i <  selected_array->length; i++) {
                                ...
                                if (ed_entity.type == EDITOR_ENTITY_POINT) {
                                    ...
                                    vec_push(copied_faces, face);
                                }
                            }

                            vec_deinit(&added_points_idxs);
                            vec_deinit(&new_points_idxs);
                        }
                    }
                        ...
                        if (ce->edit_terrain_model.active) {
                            ...
                            for (int i = 0; i < copied_points->length; i++) {
                                ...
                                if (!editor_entity_array_contains_entity(selected_array, entity)) {
                                    vec_push(selected_array, entity);
                                }
                            }
                    ...
                    if (igMenuItemBool("Duplicate", "Ctrl+D", false, true) ||
                        ...
                        if (ce->edit_terrain_model.active) {
                            ...
                            vec_editor_entity_t new_editor_entities;
                            vec_init(&new_editor_entities);
                            vec_editor_entity_t *selected_array = &ce->selected_array;
                            ...
                            for (int i = 0; i < selected_array->length; i++) {
                                ...
                                if (editor_entity.type == EDITOR_ENTITY_TERRAIN) {
                                    ...
                                    vec_push(&ce->hole->multi_terrain_entities, new_entity);
                                    ...
                                    vec_push(&new_editor_entities, new_ed_static_entity);
                                    struct editor_entity new_ed_moving_entity =
                                        make_editor_entity(EDITOR_ENTITY_MULTI_TERRAIN_MOVING,
                                                ce->hole->multi_terrain_entities.length - 1);
                                    vec_push(&new_editor_entities, new_ed_moving_entity);
                                }
                                    ...
                                    vec_push(&ce->hole->environment_entities, new_entity);
                            ...
                            for (int i = 0; i < new_editor_entities.length; i++) {
                                vec_push(selected_array, new_editor_entities.data[i]);
                            }

                            vec_deinit(&new_editor_entities);
                        }
            ...
            if (igTreeNodeStr("Entities")) {
                if (igButton("Create Terrain Entity", (ImVec2){0, 0})) {
                    ...
                    vec_push(&ce->hole->terrain_entities, entity);
                }
                ...
                if (igButton("Create Camera Zone Entity", (ImVec2){0, 0})) {
                    ...
                    vec_push(&ce->hole->camera_zone_entities, entity);
                }
                ...
                for (int i = 0; i < ce->hole->terrain_entities.length; i++) {
                    ...
                    if (igSelectableBool(label, selected, 0, (ImVec2){0, 0})) {
                        ce->object_modifier.is_active = false;
                        ce->selected_array.length = 0;
                        vec_push(&ce->selected_array, entity);
                    }
                }
        ...
        if (ce->edit_terrain_model.active) {
            ...
            if (igButton("Generate Point", (ImVec2){0, 0})) {
                ...
                vec_push(selected_array, make_editor_entity(EDITOR_ENTITY_POINT, idx));
            }
            ...
            if (selected_array->length > 0) {
                ...
                if (only_points_selected && selected_array->length == 3) {
                    if (igButton("Generate Face", (ImVec2){0, 0})) {
                        ...
                        selected_array->length = 0;
                        hovered_array->length = 0;
                        vec_push(selected_array, make_editor_entity(EDITOR_ENTITY_FACE, idx));
                    }
                }
                ...
                if (only_points_selected && selected_array->length >= 3) {
                    if (igButton("Generate Border", (ImVec2){0, 0})) {
                        ...
                        vec_vec2_t polygon_points;
                        vec_init(&polygon_points);

                        for (int i = 0; i < selected_array->length; i++) {
                            vec3 point = model->points.data[selected_array->data[i].idx];
                            vec_push(&polygon_points, V2(point.z, point.x));
                        }
                        ...
                        for (int i = 0; i < polygon_points.length; i++) {
                            ...
                            vec_push(&point_idxs, i0);
                            vec_push(&point_idxs, i1);
                            vec_push(&point_idxs, i2);
                            vec_push(&point_idxs, i3);
                        }
                ...
                if (igButton("Delete", (ImVec2){0, 0})) {
                    ...
                    vec_editor_entity_t delete_entities;
                    vec_init(&delete_entities);
                    for (int i = 0; i < selected_array->length; i++) {
                        struct editor_entity entity = selected_array->data[i];
                        if (entity.type == EDITOR_ENTITY_POINT) {
                            if (!editor_entity_array_contains_entity(&delete_entities, entity)) {
                                vec_push(&delete_entities, entity);
                            }
                            ...
                            for (int j = 0; j < model->faces.length; j++) {
                                ...
                                if (face.x == entity.idx || face.y == entity.idx || face.z == entity.idx ||
                                    ...
                                    if (!editor_entity_array_contains_entity(&delete_entities, face_entity)) {
                                        vec_push(&delete_entities, face_entity);
                                    }
                                }
                            }
                        }
            ...
            if (ce->selected_array.length > 0) {
                ...
                if (entity.type == EDITOR_ENTITY_TERRAIN) {
                    ...
                    if (igButton("Delete", (ImVec2){0, 0})) {
                        ...
                        vec_splice(&ce->hole->terrain_entities, entity.idx, 1);
                        ce->selected_array.length = 0;
                        ce->hovered_array.length = 0;
                    }
                }
                    ...
                    if (igButton("Delete", (ImVec2){0, 0})) {
                        ...
                        vec_splice(&ce->hole->multi_terrain_entities, entity.idx, 1);
                        ce->selected_array.length = 0;
                        ce->hovered_array.length = 0;
                    }
                    ...
                    if (delete_it) {
                        ...
                        vec_splice(&ce->hole->environment_entities, entity.idx, 1);
                        ce->selected_array.length = 0;
                        ce->hovered_array.length = 0;
                    }
                }
                    ...
                    if (igButton("Delete", (ImVec2){0, 0})) {
                        ...
                        vec_splice(&ce->hole->water_entities, entity.idx, 1);
                        ce->selected_array.length = 0;
                        ce->hovered_array.length = 0;
                    }
                }
...
void game_editor_init(struct game_editor *ed, struct game *game, struct renderer *renderer) {
    ...
    ed->history_idx = 0;
    vec_init(&ed->history);
    vec_init(&ed->physics.collision_data_array);
...

    {
        ...
        vec_init(&ed->ball_movement.positions);
        vec_init(&ed->ball_movement.num_ticks);
    }
...

        {
            ...
            if (game_editor_window_open) {
                ...
                if (igTreeNodeStr("Profiler")) {
                    ...
                    vec_profiler_section_info_t info_array;
                    vec_init(&info_array);
...
void game_editor_push_physics_contact_data(struct game_editor *ed, struct ball_contact contact) {
    ...
    struct physics_collision_data *collision_data = &vec_last(&ed->physics.collision_data_array);
...
void game_editor_push_physics_collision_data(struct game_editor *ed, int tick_idx,
    ...
    vec_push(&ed->physics.collision_data_array, data);
}
...
void game_editor_push_game_history_rest(struct game_editor *ed, vec3 position) {
    ...
    vec_insert(&ed->history, ed->history_idx, history);
    ed->history_idx++;
}
...
void game_editor_push_ball_movement(struct game_editor *ed, int num_ticks) {
    ...
    vec_push(&ed->ball_movement.positions, ed->game->player_ball.draw_position); 
    vec_push(&ed->ball_movement.num_ticks, num_ticks); 
}
