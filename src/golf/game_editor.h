...
typedef vec_t(struct editor_entity) vec_editor_entity_t;
...
struct hole_editor_state {
    ...
    vec_editor_entity_t selected_array;
};
typedef vec_t(struct hole_editor_state) vec_hole_editor_state_t;

struct model_generator_script {
    mfiletime_t load_time;
    mfile_t file; 
    vec_char_ptr_t params;
    vec_float_t values;

    long long sc_env_loc;
    void *sc_env;
};
typedef vec_t(struct model_generator_script) vec_model_generator_script_t;
...
struct hole_editor {
    ...
    struct {
        int idx;
        vec_hole_editor_state_t state_array;
    } history;
    ...
    vec_editor_entity_t object_modifier_entities;
    vec_vec3_t object_modifier_start_positions; 
    vec_quat_t object_modifier_start_orientations; 
    ...
    struct {
        ...
        vec_vec3_t copied_points;
        vec_terrain_model_face_t copied_faces;
    } edit_terrain_model;

    vec_model_generator_script_t scripts;
    void *sc_state; 
...
typedef vec_t(struct game_history) vec_game_history_t;
...
typedef vec_t(struct physics_collision_data) vec_physics_collision_data_t;
...
struct game_editor {
    ...
    struct {
        ...
        vec_physics_collision_data_t collision_data_array;
